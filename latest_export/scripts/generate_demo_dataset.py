...
FIELD_END_DEFAULT = datetime.fromisoformat("2026-05-22T23:59:59+01:00")
...
def generate(seed=SEED, field_start=FIELD_START_DEFAULT, field_end=FIELD_END_DEFAULT):
    ...
    # --- Step 1+2: Missing-day handler (detects any empty dates in window, synthesizes rows) ---
    MISSING_DAY_UPLOAD_OVERRIDES = {"2026-05-22": "UPL-GW01-20260523-053451"}
    MISSING_DAY_BATCH_RECEIVED_AT = "2026-05-23T06:35:46+01:00"

    window_dates = set()
    cur_d = field_start.date()
    while cur_d <= field_end.date():
        window_dates.add(cur_d.isoformat())
        cur_d += timedelta(days=1)
    covered_dates = {r["measured_at"][:10] for r in reconstructed}
    empty_dates = sorted(window_dates - covered_dates)

    missing_day_gen_rows = []
    _node_type_map = {"MAIN": "soil", "N2": "soil", "N3": "weather"}
    for missing_day in empty_dates:
        upload_id = MISSING_DAY_UPLOAD_OVERRIDES.get(missing_day, f"UPL-GW01-{missing_day.replace('-','')}T000000")
        for node in ["MAIN", "N2", "N3"]:
            node_type_str = _node_type_map[node]
            for slot in range(28):  # 07:00-20:30 every 30 min = 28 slots
                hr = 7 + slot // 2
                mn = 30 * (slot % 2)
                ts = datetime.fromisoformat(f"{missing_day}T{hr:02d}:{mn:02d}:00+01:00")
                ts_iso = ts.isoformat()
                ts_key = ts.strftime("%Y%m%d%H%M%S")
                ts_utc_str = (ts.replace(tzinfo=None) - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
                rec_id = f"GW01-{node}-{node_type_str.upper()}-MD{ts_key}"
                row_id = deterministic_uuid("missing_day_reading", node, ts_iso)
                methods_used = set()
                vals = {}
                for c in REQ[node]:
                    val = None
                    if hr in med[node] and c in med[node][hr]:
                        val = med[node][hr][c]
                        methods_used.add("missing_day_node_hourly_pattern")
                    elif c in stats[node]:
                        lo, hi = stats[node][c]
                        val = rng.uniform(lo, hi)
                        methods_used.add("missing_day_bounded_fallback")
                    if val is not None and c in stats[node]:
                        lo, hi = stats[node][c]
                        val = max(lo, min(hi, val + rng.uniform(-0.03, 0.03)))
                    vals[c] = fmt(val) if val is not None else ""
                method_str = ";".join(sorted(methods_used)) if methods_used else "missing_day_bounded_fallback"
                confidence = "medium" if "missing_day_node_hourly_pattern" in methods_used else "low"
                payload_data = {"type": "reading", "status": "ok", "node_id": node, "node_type": node_type_str,
                                "record_id": rec_id, "gateway_id": "GW01", "measured_at": ts_utc_str,
                                "battery_status": "not_measured"}
                for c in REQ[node]:
                    if vals[c]:
                        try:
                            payload_data[c] = float(vals[c])
                        except Exception:
                            pass
                new_row = {col: "" for col in sensor_cols}
                new_row["id"] = row_id
                new_row["record_id"] = rec_id
                new_row["upload_id"] = upload_id
                new_row["gateway_id"] = "GW01"
                new_row["node_id"] = node
                new_row["node_type"] = node_type_str
                new_row["measured_at"] = ts_iso
                new_row["received_at"] = MISSING_DAY_BATCH_RECEIVED_AT
                new_row["battery_status"] = "not_measured"
                for c in REQ[node]:
                    new_row[c] = vals[c]
                new_row["status"] = "ok"
                new_row["raw_payload"] = json.dumps(payload_data)
                new_row["created_at"] = MISSING_DAY_BATCH_RECEIVED_AT
                missing_day_gen_rows.append(new_row)
                audit.append({"audit_id": str(audit_id), "original_id": "", "demo_id": row_id, "node_id": node,
                             "measured_at": ts_iso, "changed_fields": ";".join(REQ[node]),
                             "original_status": "absent", "demo_status": "ok", "method": method_str,
                             "confidence": confidence, "source_rows_used": "", "source_nodes_used": "",
                             "reason": f"day {missing_day} absent from source; synthesized from hourly medians",
                             "limitations": "fully synthetic; not field-truth",
                             "original_source": "none_absent_from_source"})
                audit_id += 1
    reconstructed.extend(missing_day_gen_rows)
    # Rebuild index to include generated missing-day rows before error injection.
    by_day_node = defaultdict(list)
    for i, r in enumerate(reconstructed):
        by_day_node[(r["measured_at"][:10], r["node_id"])].append(i)

    # --- Step 3: Controlled error injection (≥3 errors, distributed across days/nodes) ---
    _ERROR_TARGETS = [
        ("2026-05-22", "MAIN", 3, "CHECKSUM_ERROR"),
        ("2026-05-19", "N2", 5, "NODE_TIMEOUT"),
        ("2026-05-18", "N3", 2, "SENSOR_READ_ERROR"),
        ("2026-05-17", "MAIN", 7, "FRAME_PARSE_ERROR"),
    ]
    controlled_errors_injected = []
    for err_day, err_node, err_slot_idx, err_code in _ERROR_TARGETS:
        ok_idxs = [i for i in by_day_node.get((err_day, err_node), []) if reconstructed[i]["status"] == "ok"]
        if not ok_idxs:
            continue
        target_i = ok_idxs[err_slot_idx % len(ok_idxs)]
        r = reconstructed[target_i]
        old_st = r["status"]
        r["status"] = "error"
        r["error_code"] = err_code
        for c in REQ[err_node]:
            r[c] = ""
        try:
            _p = json.loads(r.get("raw_payload") or "{}")
            _p["status"] = "error"
            _p["error_code"] = err_code
            for c in REQ[err_node]:
                _p.pop(c, None)
            r["raw_payload"] = json.dumps(_p)
        except Exception:
            pass
        controlled_errors_injected.append((err_day, err_node, err_code, r["measured_at"]))
        audit.append({"audit_id": str(audit_id), "original_id": r.get("id", ""), "demo_id": r.get("id", ""),
                     "node_id": err_node, "measured_at": r["measured_at"],
                     "changed_fields": ";".join(REQ[err_node]), "original_status": old_st,
                     "demo_status": "error", "method": "controlled_error_injection", "confidence": "high",
                     "source_rows_used": "", "source_nodes_used": "",
                     "reason": f"controlled error injection for dashboard error-state testing; code={err_code}",
                     "limitations": "synthetic demo scenario; not field-truth",
                     "original_source": "controlled_injection"})
        audit_id += 1

    # --- Step 4: Telemetry failure lifecycle injection ---
    RECOVERY_START_AFTER_MISSING = 3
    RECOVERY_FAIL_AFTER_MISSING = 5
    NODE_MARK_DEAD_AFTER_MISSING = 6

    def _find_ok_run(day_idxs, recs, run_len, skip_start=10):
        """Return first run of run_len consecutive ok rows after skipping skip_start rows."""
        sorted_idxs = sorted(day_idxs)
        candidates = sorted_idxs[skip_start:]
        run = []
        for idx in candidates:
            if recs[idx]["status"] == "ok":
                run.append(idx)
                if len(run) == run_len:
                    return run
            else:
    ...
    _LC_CFG = [
        ("2026-05-15", "N3", 6, True, "A"),   # full lifecycle: missing→recovery→dead→back_online
        ("2026-05-16", "N2", 5, False, "B"),   # partial lifecycle: missing→recovery→failed
    ]
    _lc_runs = {}
    for _lc_day, _lc_node, _lc_run_len, _lc_has_dead, _lc_seq in _LC_CFG:
        _day_idxs = by_day_node.get((_lc_day, _lc_node), [])
        _run = _find_ok_run(_day_idxs, reconstructed, _lc_run_len)
        if _run is None:
            continue
        _lc_runs[_lc_seq] = (_lc_node, _lc_run_len, _lc_has_dead, _run)
        for _lc_slot, _ridx in enumerate(_run):
            r = reconstructed[_ridx]
            old_st = r["status"]
            r["status"] = "missing"
            r["error_code"] = "NODE_TELEMETRY_MISSING"
            for _c in REQ[_lc_node]:
                r[_c] = ""
            try:
                _p = json.loads(r.get("raw_payload") or "{}")
                _p["status"] = "missing"
                _p["error_code"] = "NODE_TELEMETRY_MISSING"
                for _c in REQ[_lc_node]:
                    _p.pop(_c, None)
                r["raw_payload"] = json.dumps(_p)
            except Exception:
            ...
            audit.append({
                ...
                "changed_fields": ";".join(REQ[_lc_node]),
                "original_status": old_st,
                "demo_status": "missing",
                "method": "controlled_recovery_lifecycle_injection",
                "confidence": "high",
                "source_rows_used": "",
                "source_nodes_used": "",
                "reason": f"lifecycle seq {_lc_seq}: slot {_lc_slot + 1}/{_lc_run_len}",
                "limitations": "synthetic demo telemetry lifecycle; not field-truth",
                "original_source": "controlled_injection",
            })
            audit_id += 1
    assert _lc_runs, "No lifecycle runs found — check _LC_CFG targets"
    ...
    # Inject deterministic lifecycle events (recovery_started, recovery_failed, node_marked_dead).
    _LC_SEVERITY = {
        "recovery_started": "warning",
        "recovery_failed": "error",
        "node_marked_dead": "error",
    }
    _LC_MSG = {
        "recovery_started": "Node missed cycle — starting recovery watch.",
        "recovery_failed": "Recovery failed — node presumed offline.",
        "node_marked_dead": "Node exceeded dead threshold.",
    }
    lc_extra_events = []
    for _lc_seq, (_lc_node, _lc_run_len, _lc_has_dead, _lc_idxs) in _lc_runs.items():
        _lc_times = [reconstructed[i]["measured_at"] for i in _lc_idxs]
        _lc_uploads = [reconstructed[i].get("upload_id", "") for i in _lc_idxs]
        _thresholds = [
            (RECOVERY_START_AFTER_MISSING, "recovery_started"),
            (RECOVERY_FAIL_AFTER_MISSING, "recovery_failed"),
        ]
        if _lc_has_dead:
            _thresholds.append((NODE_MARK_DEAD_AFTER_MISSING, "node_marked_dead"))
        for _thresh, _etype in _thresholds:
            if _thresh > len(_lc_times):
                continue
            _ts = _lc_times[_thresh - 1]
            _up = _lc_uploads[_thresh - 1]
            _dt = parse_dt(_ts)
            _ts_key = _dt.strftime("%Y%m%d-%H%M%S")
            event_id_seq[(_lc_node, _ts_key)] += 1
            _seq_n = event_id_seq[(_lc_node, _ts_key)]
            lc_extra_events.append({
                "id": str(next_id),
                "event_id": f"EVT-GW01-{_ts_key}-{_lc_node}-{_seq_n:04d}",
                "upload_id": _up,
                "gateway_id": "GW01",
                "node_id": _lc_node,
                "event_type": _etype,
                "severity": _LC_SEVERITY[_etype],
                "event_time": _ts,
                "received_at": _ts,
                "message": _LC_MSG[_etype],
                "details": json.dumps({"sequence": _lc_seq, "consecutive_missing": _thresh}),
                "error_code": "NODE_TELEMETRY_MISSING",
            })
            next_id += 1
    out_events.extend(lc_extra_events)
    out_events.sort(key=lambda x: parse_dt(x["event_time"]))
    ...
    acols = ["audit_id", "original_id", "demo_id", "node_id", "measured_at", "changed_fields", "original_status", "demo_status", "method", "confidence", "source_rows_used", "source_nodes_used", "reason", "limitations", "original_source"]
    ...
    (OUT_DIR / "demo_generation_rules.md").write_text(
        "Demo data is synthetic/derived for dashboard use only. Raw CSV files are untouched. Uploads are unchanged. Not field-truth evidence.\n"
        "\n"
        "Missing-day handling: 2026-05-22 had zero source readings and was fully synthesised from hourly medians (method: missing_day_node_hourly_pattern / missing_day_bounded_fallback). "
        "All generated rows are assigned to upload UPL-GW01-20260523-053451 and are fully documented in demo_generation_audit.csv (original_status=absent, original_source=none_absent_from_source).\n"
        "\n"
        "Controlled errors: 4 rows were intentionally set to status=error (CHECKSUM_ERROR, NODE_TIMEOUT, SENSOR_READ_ERROR, FRAME_PARSE_ERROR) across 4 days and 3 nodes for dashboard error-state testing. "
        "Each injected error is documented in demo_generation_audit.csv (method=controlled_error_injection, original_source=controlled_injection).\n"
        "\n"
        "Telemetry failure lifecycle: Two deterministic recovery sequences are injected into system_events_demo.csv. "
        "Sequence A (N3, 2026-05-15): 6 consecutive rows marked missing → events: recovery_started (after 3rd), recovery_failed (after 5th), node_marked_dead (after 6th), node_back_online (organic). "
        "Sequence B (N2, 2026-05-16): 5 consecutive rows marked missing → events: recovery_started (after 3rd), recovery_failed (after 5th). "
        "All lifecycle rows are audited with method=controlled_recovery_lifecycle_injection.\n",
        encoding="utf-8")
    (OUT_DIR / "dashboard_demo_notes.md").write_text(
        "Dataset is for dashboard demo and QC visualization only. Synthetic missingness and reconstructions were applied.\n"
        "\n"
        "2026-05-22 coverage: This day was absent from the source data. 84 rows (28 slots x 3 nodes, 07:00-20:30 at 30-min intervals) were synthesised using hourly medians "
        "from the 2026-05-12 to 2026-05-21 window and assigned to the 2026-05-23 batch upload (UPL-GW01-20260523-053451). "
        "These rows are labelled original_status=absent in the audit trail and must not be treated as field measurements.\n"
        "\n"
        "Controlled errors: 4 sensor readings were deliberately injected with realistic error codes (CHECKSUM_ERROR on 2026-05-22/MAIN, NODE_TIMEOUT on 2026-05-19/N2, "
        "SENSOR_READ_ERROR on 2026-05-18/N3, FRAME_PARSE_ERROR on 2026-05-17/MAIN) to exercise the dashboard error-state display. "
        "Corresponding node_missing system events and node_back_online recovery events are present in system_events_demo.csv.\n"
        "\n"
        "Telemetry failure lifecycle: system_events_demo.csv contains two injected recovery sequences. "
        "Sequence A — N3 on 2026-05-15: 6 consecutive readings marked missing, triggering recovery_started (warning, after 3rd), recovery_failed (error, after 5th), node_marked_dead (error, after 6th), and an organic node_back_online (info) when telemetry resumed. "
        "Sequence B — N2 on 2026-05-16: 5 consecutive readings marked missing, triggering recovery_started (warning, after 3rd) and recovery_failed (error, after 5th). "
        "All lifecycle sensor rows carry error_code=NODE_TELEMETRY_MISSING and are audited under method=controlled_recovery_lifecycle_injection in demo_generation_audit.csv.\n",
        encoding="utf-8")
    ...
    assert all("2026-05-12" <= r["date"] <= "2026-05-22" for r in qrows)
    ...
    return {"field_start": field_start.isoformat(), "field_end": field_end.isoformat(),
            ...
            "missing_day_rows_added": len(missing_day_gen_rows),
            "controlled_errors_injected": len(controlled_errors_injected)}
